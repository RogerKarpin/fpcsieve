/* app.c -- (C) March 2009, Mark Rodenkirch, Geoffrey Reynolds.
                Januray 2015, Roger Karpin

   Factorial/Primorial/Compositorial sieve application.
...
#define MODE_COMPOSITORIAL 3

unsigned int *sieve_composites = NULL; /* Master list of composites */
...
static int mode = MODE_NONE, prime_base = 0, prime_count = 0, composite_base = 0, composite_count = 0;
...
void free_sieve_composites(void)
{
  if (sieve_composites != NULL)
  {
    free(sieve_composites);
    sieve_composites = NULL;
  }
}
...
{
  printf("fpcsieve version " APP_VERSION " (testing)\n");
#ifdef __GNUC__
...
{
  size_t bytes;
  unsigned int i, j, last_prime = 3, next_prime = 5;
  ...
  else
  {
	...
	if (ch1 == '!')
	{
	  ...
	  {
		if (mode == MODE_PRIMORIAL || mode == MODE_COMPOSITORIAL)
		{
		  fprintf(stderr,"%s mode selected but input file `%s' is factorial\n",(mode == MODE_PRIMORIAL)? "primorial":"compositorial",input_filename);
		  exit(EXIT_FAILURE);
		}
	  ...
	  {
		if (mode == MODE_FACTORIAL || mode == MODE_PRIMORIAL)
		{
		  fprintf(stderr,"%s mode selected but input file `%s' is compositorial\n",(mode == MODE_FACTORIAL)? "factorial":"primorial",input_filename);
		  exit(EXIT_FAILURE);
		}
		else if (fscanf(file,"$a%c+$%c%*[^\n]",&ch1,&ch2) != 2 || (ch1 != '#' && ch2 != 'b'))
		{
	      fprintf(stderr,"Invalid header in input file `%s'\n",input_filename);
          exit(EXIT_FAILURE);
	    }
	...
	{
	  if (mode == MODE_FACTORIAL || mode == MODE_COMPOSITORIAL)
      {
        fprintf(stderr,"%s mode selected but input file `%s' is primorial\n",(mode == MODE_FACTORIAL)? "factorial":"compositorial",input_filename);
        exit(EXIT_FAILURE);
      }
	  else if (fscanf(file,"+$%c%*[^\n]",&ch2) != 1 || ch2 != 'b')
	  {
  ...
  else
  {
    ...
    for (prime_count = 0; sieve_primes[prime_count] <= nmax; prime_count++)
	  ;
	if (mode == MODE_COMPOSITORIAL)
	{
	  int composites_to_reserve = 0;

	  composites_to_reserve = nmax-prime_count-1;
	  if (sieve_composites == NULL)
		sieve_composites = xmalloc((composites_to_reserve+1)*sizeof(unsigned int));
	  sieve_composites[composites_to_reserve] = -1U;
	  //printf("composites_to_reserve = %u, -1U = %u\n",composites_to_reserve,-1U);
	  for (i = 0; sieve_primes[i] <= nmax; i++)
		if (composite_count < composites_to_reserve)
		{
		  next_prime = sieve_primes[i];
		  for (j = last_prime + 1; (j < next_prime) && (composite_count < composites_to_reserve); j++)
			sieve_composites[composite_count++] = j;
		  last_prime = next_prime;
		}
	  for (i = last_prime + 1; i <= nmax; i++)
		  sieve_composites[composite_count++] = i;
	  //printf("prime_base = %u, prime_count = %u, sieve_primes[prime_base] = %u, sieve_primes[prime_count-1] = %u\n",prime_base,prime_count,sieve_primes[prime_base],sieve_primes[prime_count-1]);
	  //printf("composite_count = %u\n",composite_count);
	  lim_sieve_composites = sieve_composites[composite_count-1];
	  //printf("sieve_composites[composite_count-1] = lim_sieve_composites = %u\n",lim_sieve_composites);
	  //for (i = 0; i < 114; i++)
		//printf("sieve_composites[%u] = %u\n",i,sieve_composites[i]);
	  for (composite_base = 0; sieve_composites[composite_base] < nmin; composite_base++)
		;
	  //printf("composite_base = %u\n",composite_base);
	  //printf("sieve_composites[composite_base] = %u\n",sieve_composites[composite_base]);
	  bytes = (composite_count-composite_base+7)/8;
	}
  ...
  if (input_filename == NULL)
  {
    ...
    if (mode == MODE_FACTORIAL)
	  ...
	  remaining_terms = 2*(composite_count-composite_base);
  }
  else
  {
	FILE *file;
    ...
    while (fscanf(file," %u %d",&n,&c)==2)
    {
	  ...
	  else if (mode == MODE_COMPOSITORIAL)
	  {
		/* This assumes that n is non-decreasing */
        for ( ; sieve_composites[composite_base+bit] < n; bit++)
          ;
        if (sieve_composites[composite_base+bit] != n)
        {
          fprintf(stderr,"non-composite %u in input file `%s'\n",n,input_filename);
		  fprintf(stderr,"composite_base = %u, bit = %u, sieve_composites[composite_base+bit] = %u\n",composite_base,bit,sieve_composites[composite_base+bit]);
          exit(EXIT_FAILURE);
        }
		//else
			//printf("OK. composite_base = %u, bit = %u, sieve_composites[composite_base+bit] = %u\n",composite_base,bit,sieve_composites[composite_base+bit]);
	  }
  ...
  printf("%s sieve initialized: %u <= n <= %u\n",
         (mode == MODE_FACTORIAL)? "factorial":
		 (mode == MODE_PRIMORIAL)? "primorial":"compositorial",nmin,nmax);
}
      ...
      for (i = 0; i < prime_count; i++)
      {
        ...
        if (n >= nmin)
        {
          for (j = 0; j < APP_BUFLEN; j++)
          {
            if (REM[j] ==      1)
			{
			  //printf("-1 composite = %u, REM[%u] = %u == 1\n",i,j,REM[j]);
			  report_factor(i, -1, P[j]);
			}
            if (REM[j] == P[j]-1)
			{
			  //printf("+1 composite = %u, REM[%u] = %u == P[%u] -1 = %u - 1\n",i,j,REM[j],j,P[j]);
			  report_factor(i,  1, P[j]);
			}
          }
        }
      ...
      for (i = 0; i < composite_count; i++)
      {
        ...
        if (n >= nmin)
        {
          for (j = 0; j < APP_BUFLEN; j++)
          {
			...
			{
			  //printf("-1 composite = %u, REM[%u] = %u == 1\n",i,j,REM[j]);
			  //printf("nmin = %u, n = %u, sieve_composites[i] = %u, composite_count = %u, nmax = %u\n",nmin,n,sieve_composites[i], composite_count, nmax);
			  report_factor(i, -1, P[j]);
			}
            if (REM[j] == P[j]-1)
			{
...
void app_write_checkpoint(FILE *fout)
{
  ...
  if (output_filename != NULL)
  {
	...
	else /* mode == MODE_COMPOSITORIAL */
    {
      ...
      {
        if (minus_list[i/8] & (1 << i%8))
          fprintf(file,"%u -1\n",sieve_composites[composite_base+i]), count++;
        if (plus_list[i/8] & (1 << i%8))
          fprintf(file,"%u +1\n",sieve_composites[composite_base+i]), count++;
      }
    }
